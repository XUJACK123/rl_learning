...
- 价值网络更新： $\phi$：只用: $$\mathcal{L}^{\text{VF}}(\phi) = \frac{1}{2} \mathbb{E} \left[ \left( V_\phi(s_t) - G_t \right)^2 \right]$$的均方误差损失 $L^{VF}(\phi)$ 去做梯度更新
    - 预测值 $V_\phi(s_t)$：在状态$s_t$下，critic网络所预测的value(是value不是reward，跑完整个游戏后的奖励)
    - 真实目标 $G_t$：真实跑完之后能获得多少分
    - 平方能使得误差永远是正数
    - E代表整批采样中所有的时间步的平均损失
...
- σ也是一种可学习参数，也是一种慢慢衰减的ε，如果梯度发现探索不够，损失还大就会调大σ，如果找到门道的关节，梯度把它压小，动作变得精准
    - 在神经网络中，算法并不是直接输出 $\sigma$，而是输出 $\log \sigma$(避免 $\sigma \le 0$)
    - 通过对σ求导 $$\frac{\partial \log \pi_\theta(a \mid s)}{\partial \sigma} = \frac{1}{\sigma} \left[ \underbrace{\left( \frac{a - \mu}{\sigma} \right)^2}_{\text{标准化探索偏差}} - 1 \right]$$
    - 带入导数代入策略梯度：$$\text{Gradient}_\sigma \propto \left[ \left( \frac{a - \mu}{\sigma} \right)^2 - 1 \right] \cdot A_t$$
    - 中间项是代表尝试的动作a距离均值$\mu$有多远
    1. 如果结果>1，$A_t > 0$代表尝试了一个很远的动作，拿到正奖励，梯度变大
    2. 如果结果<1，$A_t > 0$代表动作本身就是在预测中心附近，收窄 $\sigma$，让动作变得更加精准稳定
    3. 如果结果>1，$A_t < 0$代表动作尝试错了，收窄 $\sigma$
    4. 如果结果<1，$A_t < 0$代表这个动作有问题，加大探索$\sigma$
...
## 熵价值
- 神经网络的末端是softmax，会放大指数的特性，使得有些动作因为偶然的噪声运气好拿到了一点点奖励就会无限放大
- 熵的作用：阻止动作过早变的很低，在开始的时候让每一个动作尽量平均一点，在奖励变多后就不用在乎熵的那点惩罚了
...
- 那是如何判断哪些action需要加强哪些action不用加强呢：
    - 在奖励明确的地方(做了这个action能带来明确的收益)：$\text{策略梯度} \gg \text{熵梯度}$，选择该动作的概率会变得很大(策略梯度调整的)，熵的值自然会变得很低(熵的定义)
    - 在奖励不明确的地方，策略梯度较低，选择不同动作的概率都差不多，熵会很大
...
- 共享主干: 如经典 RL、机器人控制、视觉游戏
    ...
    - 总体的更新：$$\text{Loss}_{\text{total}} = \text{Loss}_{\text{policy}} + c_1 \cdot \text{Loss}_{\text{value}} - c_2 \cdot H(\pi_\theta)$$
